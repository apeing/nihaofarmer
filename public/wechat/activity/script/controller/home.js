...
(function(angular){
    ...
    angular.module('activity').controller('HomeController',
        ['$scope','$routeParams', '$location', '$http', '$window','busyIndicatorService','$qupload','alertMessageService',
            function($scope,$routeParams, $location, $http, $window,busyIndicatorService,$qupload,alertMessageService){
                $scope.logout = function(){
                    $window.location.href = '/cookies-mobile.html';
                }
                ...
                $scope.uploadImage = function(files) {
                    if (files && files.length) {
                        ...
                        files.forEach(function(item){
                            ...
                            $http.get('/qiniu/qupload').success(function(resobj) {
                                var upload = $qupload.upload({
                                    key: 'nihaofarmer/' + $scope.mobile + '/' + file.name,
                                    file: file,
                                    token: resobj.token
                                });
